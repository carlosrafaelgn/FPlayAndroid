...
public final class OpenSLMediaPlayer implements IBasicMediaPlayer {
	...
	public OpenSLMediaPlayer(OpenSLMediaPlayerContext context) {
		if (context == null)
		...
		try {
			mNativeHandle = createNativeImplHandle(contextHandle, new WeakReference<>(this), new int[1]);
		} catch (Throwable ex) {
			ex.printStackTrace();
		}
	...
	public int getCurrentPosition() {
		...
		try {
			...
			Log.e(TAG, "An error occurred in getCurrentPosition()", e);
		}
	...
	public void attachAuxEffect(int effectId) {
		...
		try {
			...
			Log.e(TAG, "An error occurred in attachAuxEffect(effectId = " + effectId + ")", e);
		}
	}
	...
	public void setVolume(float leftVolume, float rightVolume) {
		...
		try {
			...
			Log.e(TAG, "An error occurred in setVolume(leftVolume = " + leftVolume + ", rightVolume = " + rightVolume + ")", e);
		}
	}
	...
	public void setAudioStreamType(int streamtype) {
		...
		try {
			...
			Log.e(TAG, "An error occurred in setAudioStreamType(streamtype = " + streamtype + ")", e);
		}
	}
	...
	public void setAuxEffectSendLevel(float level) {
		...
		try {
			...
			Log.e(TAG, "An error occurred in setAuxEffectSendLevel(level = " + level + ")", e);
		}
	}
	...
	public void setNextMediaPlayer(IBasicMediaPlayer next) {
		...
		try {
			...
			Log.e(TAG, "An error occurred in setNextMediaPlayer(nextHandle = " + nextHandle + ")", e);
		}
	}
	...
	//
	private void setDataSourceInternalNonContentUri(Context context, Uri uri) throws IOException {
	...
	private void setDataSourceInternalContentUri(Context context, Uri uri) throws IOException {
		...
		final int result = ((declLength < 0) ? setDataSourceFdImplNative(mNativeHandle, nativeFD) :
			setDataSourceFdImplNative(mNativeHandle, nativeFD, startOffset, declLength));
	...
	private void createWakeLockObject(Context context, int mode) {
		...
		WakeLock wakelock = pm.newWakeLock(mode | PowerManager.ON_AFTER_RELEASE, WAKELOCK_TAG);
	...
	// Native methods
	//
	private static native long createNativeImplHandle(long contextHandle, WeakReference<OpenSLMediaPlayer> weak_thiz, int[] params);
	...
	private static native int setDataSourceFdImplNative(long handle, int fd, long offset, long length);

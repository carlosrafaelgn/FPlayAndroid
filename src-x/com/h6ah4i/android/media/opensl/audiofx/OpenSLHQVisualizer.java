...
public final class OpenSLHQVisualizer implements IHQVisualizer {
	...
	public OpenSLHQVisualizer(OpenSLMediaPlayerContext context) {
		...
		if (HAS_NATIVE)
			mNativeHandle = createNativeImplHandle(OpenSLMediaPlayer.Internal.getNativeHandle(context), new WeakReference<>(this));

		if (mNativeHandle == 0)
			throw new UnsupportedOperationException("Failed to initialize native layer");

		mParamIntBuff = new int[2];
	...
	private static void raiseCaptureEventFromNative(Object ref, int type, float[] data, int numChannels, int samplingRate) {
		//
	...
	// Native methods
	//
	private static native long createNativeImplHandle(long context_handle, WeakReference<OpenSLHQVisualizer> weak_thiz);
	...
	private static native int setDataCaptureListenerImplNative(long handle, int rate, boolean waveform, boolean fft);

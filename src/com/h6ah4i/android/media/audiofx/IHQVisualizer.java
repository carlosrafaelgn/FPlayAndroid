...
public interface IHQVisualizer extends IReleasable {
	 ...
	 */
	int SUCCESS = IAudioEffect.SUCCESS;
	/**
	 * Unspecified error.
	 */
	int ERROR = IAudioEffect.ERROR;
	/**
	 * Internal operation status. Not returned by any method.
	 */
	int ALREADY_EXISTS = IAudioEffect.ALREADY_EXISTS;
	/**
	 * Operation failed due to bad object initialization.
	 */
	int ERROR_NO_INIT = IAudioEffect.ERROR_NO_INIT;
	/**
	 * Operation failed due to bad parameter value.
	 */
	int ERROR_BAD_VALUE = IAudioEffect.ERROR_BAD_VALUE;
	/**
	 * Operation failed because it was requested in wrong state.
	 */
	int ERROR_INVALID_OPERATION = IAudioEffect.ERROR_INVALID_OPERATION;
	/**
	 * Operation failed due to lack of memory.
	 */
	int ERROR_NO_MEMORY = IAudioEffect.ERROR_NO_MEMORY;
	/**
	 * Operation failed due to dead remote object.
	 */
	int ERROR_DEAD_OBJECT = IAudioEffect.ERROR_DEAD_OBJECT;

	/**
	 * State of a Visualizer object that was not successfully initialized upon
	 * creation
	 */
	int STATE_UNINITIALIZED = android.media.audiofx.Visualizer.STATE_UNINITIALIZED;
	/**
	 * State of a Visualizer object that is ready to be used.
	 */
	int STATE_INITIALIZED = android.media.audiofx.Visualizer.STATE_INITIALIZED;
	/**
	 * State of a Visualizer object that is active.
	 */
	int STATE_ENABLED = android.media.audiofx.Visualizer.STATE_ENABLED;

	/**
	 * Rectangular window.
	 */
	int WINDOW_RECTANGULAR = 0;
	/**
	 * Hann window.
	 */
	int WINDOW_HANN = 1;
	 ...
	 * Hamming window.
	 */
	int WINDOW_HAMMING = 2;
	/**
	 * Blackman window.
	 */
	int WINDOW_BLACKMAN = 3;
	/**
	 * Flat top window.
	 */
	int WINDOW_FLAT_TOP = 4;
	/**
	 * Option flag to specify whether the window function is also effects to
	 * thecaptured waveform data.
	 */
	int WINDOW_OPT_APPLY_FOR_WAVEFORM = (1 << 31);
	...
	interface OnDataCaptureListener {
		 ...
		 */
		void onWaveFormDataCapture(IHQVisualizer visualizer, float[] waveform, int numChannels, int samplingRate);
		 ...
		 * @param samplingRate sampling rate of the audio visualized.
		 */
		void onFftDataCapture(IHQVisualizer visualizer, float[] fft, int numChannels, int samplingRate);
	}
	 ...
	 */
	int setDataCaptureListener(OnDataCaptureListener listener, int rate, boolean waveform, boolean fft);
